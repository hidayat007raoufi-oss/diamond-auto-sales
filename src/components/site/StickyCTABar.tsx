...
export default function StickyCTABar() {
  ...
  return (
    <div
      className={`fixed inset-x-0 bottom-0 z-40 border-t border-zinc-200 bg-white/95 px-3 pb-[max(0.6rem,env(safe-area-inset-bottom))] pt-2.5 backdrop-blur transition-all duration-300 lg:hidden ${
        visible ? "translate-y-0 opacity-100" : "pointer-events-none translate-y-full opacity-0"
      }`}
    >
      <div className="mx-auto flex max-w-md items-center gap-2">
        <a href={`tel:${PHONE}`} aria-label="Call" className="grid h-11 w-11 shrink-0 place-items-center rounded-full border border-zinc-300 text-zinc-900 active:scale-95">
