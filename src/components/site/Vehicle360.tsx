...
export default function Vehicle360({
  ...
  return (
    <div
      ref={wrap}
      className={`relative aspect-[4/3] touch-none select-none overflow-hidden rounded-2xl bg-[#f5f5f7] ring-1 ring-black/[0.06] ${dragging ? "cursor-grabbing" : "cursor-grab"}`}
      onPointerDown={(e) => {
    ...
    >
      ...
      {active.map((h) => (
        ...
        >
          <span className="absolute left-1/2 top-1/2 h-7 w-7 -translate-x-1/2 -translate-y-1/2 animate-ping rounded-full border border-[#0071e3]/50" />
          <span className="relative block h-2.5 w-2.5 rounded-full bg-[#0071e3]" />
          <span className="absolute left-1/2 top-5 -translate-x-1/2 whitespace-nowrap rounded-full bg-[#1d1d1f] px-2.5 py-1 text-[10px] tracking-widest text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100">
            {h.caption}
          </span>
        </button>
      ))}

      <span className="pointer-events-none absolute left-5 top-5 inline-flex items-center gap-2 rounded-full bg-[#1d1d1f] px-3 py-1 text-[11px] font-medium tracking-widest text-white">
        <svg viewBox="0 0 24 24" className="h-3.5 w-3.5" fill="none" stroke="currentColor" strokeWidth="1.5">
          <path d="M21 12a9 9 0 11-3-6.7M21 4v4h-4" />
        </svg>
        360°
      </span>
      <span className="pointer-events-none absolute bottom-5 left-1/2 -translate-x-1/2 rounded-full border border-black/10 bg-white/80 px-3 py-1 text-[11px] text-[#6e6e73] backdrop-blur">
